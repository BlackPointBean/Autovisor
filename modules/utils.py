...
async def optimize_page(page: Page, config: Config, is_new_version=False, is_hike_class=False) -> None:
    try:
        #await page.wait_for_load_state("domcontentloaded")
        await evaluate_js(page, ".studytime-div", config.pop_js, 3000, is_hike_class)
...
async def get_lesson_name(page: Page, is_hike_class=False) -> str:
    if is_hike_class:
        ...
        return title
    try:
        title_ele = await page.wait_for_selector("#lessonOrder", timeout=2000)
        return await title_ele.get_attribute("title")
    except:
        title_ele = await page.wait_for_selector(".chapter-content-second.current .item-name", timeout=2000)
        return await title_ele.text_content()
...
async def get_filtered_class(page: Page, is_new_version=False, is_hike_class=False, include_all=False) -> List[Locator]:
    if is_hike_class:
        ...
        all_class = await page.locator(".chapter-content-second").all()
        if include_all:
            logger.debug(f"Get to-review class: {len(all_class)}")
            return all_class
        to_learn_class = []
        for each in all_class:
            finish_icon = await each.locator(".finish-icon").count()
            if finish_icon > 0:
                continue
            progress = each.locator(".el-progress")
